...
namespace LibraryManagementSystem.Controllers.Api.V1
{
    ...
    public class ApiCustomersController : ControllerBase
    {
        ...
        public ApiCustomersController(ICustomerService svc) => _svc = svc;

        [HttpGet]
        public async Task<ActionResult<PagedResult<Customer>>> Get([FromQuery] string? searchTerm, [FromQuery] bool? activeOnly, [FromQuery] string? sortBy,
        ...
        public async Task<ActionResult<Customer>> Create([FromBody] Customer customer)
        {
            ...
            var created = await _svc.CreateAsync(customer);
        ...
        {
            if (!ModelState.IsValid) return ValidationProblem(ModelState);
            var ok = await _svc.UpdateAsync(id, customer);
